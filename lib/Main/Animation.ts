 ...
 * @LastEditTime: 2024-05-01 17:21:35
 */
...
export default class Animation extends commonClass {
  /**
   * @description: gltf中原始的动画数组
   * @return {*}
   * @author: 池樱千幻
   */
  gltfAnimations: any;
  /**
   * @description: 根据gltf中的动画,扩展的动画数组
   * @return {*}
   * @author: 池樱千幻
   */
   ...
   * @author: 池樱千幻
   */
  rootObject: THREE.Object3D<THREE.Event>;
  constructor(rootObject: THREE.Object3D<THREE.Event>) {
    super();
    //原始动画数组
  ...
  startAllAnimations() {
    if (this._mixer) {
      this.animationsList.forEach((item) => {
        item.ued_loop();
      });
    }
  }
